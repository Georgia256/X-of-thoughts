...
def get_chat_response(args, input, key, org_id, n=1):
    ...
    while patience > 0:
        ...
        if args.mode in [
            ...
            del model  # Delete the model to free up memory
            torch.cuda.empty_cache()
            print("final text from model: ", final_text)
            return final_text
...
def get_chat_response_rank(args, input, key, org_id, n=1):
    if org_id is not None:
        openai.organization = org_id
    patience = 100
    ...
    while patience > 0:
        ...
        if args.mode in [
            ...
            max_len = math.ceil(input_ids.shape[1] * (1 + 1 / (n_examples)))
            # attention_mask = inputs.attention_mask.to(model.device)
            outputs = model.generate(
                input_ids=input_ids,
                # attention_mask=attention_mask,
...
def safe_solve_equation_system(equations):
    ...
    def solve_equation_system(x):
        ...
        for eq in x:
            ...
            clean_eq = [
                re.sub(digit_pattern, add_multiplication, seg) for seg in eq.split()
            ]
            clean_eq = " ".join(clean_eq)
            left, right = clean_eq.replace("(", " ( ").replace(")", " ) ").split("=")
            # parse_expr(sub_eqs, transformations=transformations)
            left = [
                re.sub(pattern, add_multiplication, c) for c in left.strip().split()
            ]
            left = " ".join(left)

            right = [
                re.sub(pattern, add_multiplication, c) for c in right.strip().split()
            ]
            right = " ".join(right)
...
def get_final_using_sympy(equations):
    try:
        ...
        if (
            equation_list[-1].split("=")[0].strip().isalpha()
            or len(equation_list[-1].split("=")[0].strip()) == 2
        ):
            ...
            if goal_var is not None:
                ...
                goal_expression = parse_expr(
                    goal_expression, transformations=transformations
                )
        ...
        if len(equation_list) == 1:
            try:
                goal_expression = parse_expr(
                    equation_list[0].split("=")[0], transformations=transformations
                )
        ...
        for i in range(len(equation_list) - 1):
            ...
            if "?" not in sub_eqs:
                try:
                    ...
                    sub_eqs = (
                        sub_eqs_split[0].strip()
                        + " - ("
                        + sub_eqs_split[1].strip()
                        + ")"
                    )
