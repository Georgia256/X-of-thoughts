...
if __name__ == "__main__":
    ...
    parser.add_argument("--tot_assertion", default='', type=str)
    ...
    for i in range(data_len):
        pot, tot, eot = pot_data[i], tot_data[i], eot_data[i]
        if pot['reason/pot/score'] == 1 and tot['reason/tot/score'] == 0 and eot['reason/eot/score'] == 0:
            pot_good.append(i)
        if pot['reason/pot/score'] == 0 and tot['reason/tot/score'] == 1 and eot['reason/eot/score'] == 0:
            tot_good.append(i)
        if pot['reason/pot/score'] == 0 and tot['reason/tot/score'] == 0 and eot['reason/eot/score'] == 1:
            eot_good.append(i)
    ...
    for i in range(data_len):
        pot, tot, eot = pot_data[i], tot_data[i], eot_data[i]
        pot_score, tot_score, eot_score = pot['reason/pot/score'], tot['reason/tot/score'], eot['reason/eot/score']

        if pot_score == 1 or eot_score == 1:
            pot_or_eot += 1
        if pot_score == 1 or tot_score == 1:
            pot_or_tot += 1
        if tot_score == 1 or eot_score == 1:
            tot_or_eot += 1
        if pot_score == 1 or tot_score == 1 or eot_score == 1:
            all_or += 1

    logger.info(f"===== OR =====")
    logger.info(f"pot or eot: {pot_or_eot}, {pot_or_eot / data_len}")
    logger.info(f"pot or tot: {pot_or_tot}, {pot_or_tot / data_len}")
    logger.info(f"tot or eot: {tot_or_eot}, {tot_or_eot / data_len}")
    ...
    xot_collaboration(pot_data, tot_data, pot_assertion_data, 'pot', 'tot')

    # eot assertion
    xot_collaboration(eot_data, pot_data, eot_assertion_data, 'eot', 'pot')
    xot_collaboration(eot_data, tot_data, eot_assertion_data, 'eot', 'tot')

    # passive feedbak
    xot_collaboration_passive(pot_data, tot_data, 'pot', 'tot')
    xot_collaboration_passive(eot_data, tot_data, 'eot', 'tot')

    # eot -> pot -> tot
    assert_eot_pot_tot_acc = 0
    for i in range(data_len):
        ...
        if eot_assert:
            assert_eot_pot_tot_acc += eot_score
        elif pot_assert:
            assert_eot_pot_tot_acc += pot_score
        else:
            assert_eot_pot_tot_acc += tot_score
    logger.info(f"===== Assert eot & pot & tot =====")
    logger.info(assert_eot_pot_tot_acc / data_len)

    # pot -> eot -> tot
    assert_pot_eot_tot_acc = 0
    for i in range(data_len):
        eot_score = eot_data[i]['reason/eot/score']
        pot_score = pot_data[i]['reason/pot/score']
        tot_score = tot_data[i]['reason/tot/score']
        ...
        eot_assert = eot_assertion_data[i]['think/check/eot/flag']
        if pot_assert:
            assert_pot_eot_tot_acc += pot_score
        elif eot_assert:
            assert_pot_eot_tot_acc += eot_score
        else:
            assert_pot_eot_tot_acc += tot_score
    logger.info(f"===== Assert pot & eot & tot =====")
    logger.info(assert_pot_eot_tot_acc / data_len)
    ...
    for i in range(data_len):
        ...
        tot_ans = tot_data[i]['reason/tot/ans']

        # get the majority vote among three answers
        # Find the most frequent answer
        ans_list = [tot_ans, pot_ans, eot_ans]
        score_list = [tot_score, pot_score, eot_score]
    ...
    if plan_data is not None:
        ...
        for i in range(data_len):
            ...
            temp = plan_data[i]['plan']
            # Regular expression patterns for each rating
            python_program_pattern = r'Python_Program_rating:(\d+)'
            system_of_linear_equations_pattern = r'System_of_linear_equations_rating:(\d+)'
            chain_of_thought_pattern = r'Chain_of_Thought_rating:(\d+)'
            # Find ratings using the regular expressions
            python_program_rating = re.search(python_program_pattern, temp)
            ...
            chain_of_thought_rating *= tot_acc/data_len #0.314
            ...
            plan_list = sorted(ratings_list, key=lambda x: x[1], reverse=True)

            if plan_list[0][0] == 'System_of_linear_equations':
                if eot_assertion_data[i]['think/check/eot/flag']:
                    plan_assert_acc += eot_score
                    steps_sum += 1
                    pec_cnt['e'] += 1
                elif pot_assertion_data[i]['think/check/pot/flag'] and plan_list[1][0] == 'Python_Program':
                    plan_assert_acc += pot_score
                    steps_sum += 2
                    pec_cnt['p'] += 1
                elif (meta_eval_data[i]['metacognitive_eval/tot/ans'] == tot_data[i]['reason/tot/ans']) and plan_list[1][0] == 'Chain_of_Thought':
                    plan_assert_acc += tot_score
                    steps_sum += 2
                    pec_cnt['c'] += 1
                else:
                    if plan_list[2][0] == 'Chain_of_Thought':
                        plan_assert_acc += tot_score
                        pec_cnt['c'] += 1
                    else:
                        plan_assert_acc += pot_score
                        pec_cnt['p'] += 1
                    steps_sum += 3    
            elif plan_list[0][0] == 'Python_Program':
                if pot_assertion_data[i]['think/check/pot/flag']:
                    plan_assert_acc += pot_score
                    steps_sum += 1
                    pec_cnt['p'] += 1
                elif eot_assertion_data[i]['think/check/eot/flag'] and plan_list[1][0] == 'System_of_linear_equations':
                    plan_assert_acc += eot_score
                    steps_sum += 2
                    pec_cnt['e'] += 1
                elif (meta_eval_data[i]['metacognitive_eval/tot/ans'] == tot_data[i]['reason/tot/ans']) and plan_list[1][0] == 'Chain_of_Thought':
                ...
                if meta_eval_data[i]['metacognitive_eval/tot/ans'] == tot_data[i]['reason/tot/ans']:
                    plan_assert_acc += tot_score
                    steps_sum += 1
                    pec_cnt['c'] += 1
                elif eot_assertion_data[i]['think/check/eot/flag'] and plan_list[1][0] == 'System_of_linear_equations':
                    ...
                    if plan_list[2][0] == 'Python_Program':
                        plan_assert_acc += pot_score
                        pec_cnt['p'] += 1
                    else:
                        plan_assert_acc += eot_score
        ...
        logger.info(f"PoT: {pec_cnt['p']},{pec_cnt['p'] / data_len}, EoT: {pec_cnt['e']}, {pec_cnt['e'] / data_len},"
                    f"tot: {pec_cnt['c']},{pec_cnt['c'] / data_len}")
        ...
        for i in range(data_len):
            if tot_data[i]['reason/tot/score'] == 0:
                if meta_eval_data[i]['metacognitive_eval/tot/ans'] == tot_data[i]['reason/tot/ans']:
                    fp += 1
                else:
                    tn += 1
